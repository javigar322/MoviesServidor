...
public class SecurityConfiguration {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.csrf(AbstractHttpConfigurer::disable).authorizeHttpRequests(request -> request
				...
				.requestMatchers(HttpMethod.POST, "/api/v1/user/**").hasAuthority(Role.ROLE_ADMIN.toString())
				.requestMatchers(HttpMethod.PUT, "/api/v1/user/**").hasAuthority(Role.ROLE_ADMIN.toString())

				.requestMatchers(HttpMethod.DELETE, "/api/v1/user/**").hasAuthority(Role.ROLE_ADMIN.toString())
				.requestMatchers("/api/v1/user/**").hasAuthority("ROLE_ADMIN") // Modificado aquí
